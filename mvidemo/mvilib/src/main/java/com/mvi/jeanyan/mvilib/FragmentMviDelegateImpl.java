...
public class FragmentMviDelegateImpl<V extends MviView, P extends MviPresenter<V>>
    ...
    @Override public void onCreate(@Nullable Bundle bundle) {
        ...
        if (DEBUG) {
            Log.d(DEBUG_TAG,
                    "MosbyView ID = " + mosbyViewId + " for MviView: " + delegateCallback.getMviView());
        }
    ...
    @Override public void onStart() {
        ...
        V view = delegateCallback.getMviView();
        if (view == null) {
            throw new NullPointerException(
                    "MviView returned from getMviView() is null. Returned by " + fragment);
        }
        ...
        if (DEBUG) {
            Log.d(DEBUG_TAG,
                    "MviView attached to Presenter. MviView: " + view + "   Presenter: " + presenter);
        }
    }
    ...
    private boolean retainPresenterInstance(boolean keepPresenterOnBackstack, Activity activity,
        ...
        if (keepPresenterOnBackstack && BackstackAccessor.isFragmentOnBackStack(fragment)) { //判断是否在回退栈中
            return true;
        }
    ...
    @Override public void onStop() {
        ...
        if (DEBUG) {
            Log.d(DEBUG_TAG, "detached MviView from Presenter. MviView "
                    + delegateCallback.getMviView().toString()
                    + "   Presenter: "
                    + presenter);
        }
    }
